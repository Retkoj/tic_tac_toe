...
class GameState:
    ...
    def game_not_started(self) -> bool:
        return self.grid.space_count == len(self.grid.cells)
    ...
    def winning_cells(self) -> list[int]:
        """
        :return: list with indexes of the winning cells
        """
        for pattern in WINNING_PATTERNS:
            for mark in Mark:
                if re.match(pattern.replace("?", mark), self.grid.cells):
    ...
    def make_move_to(self, cell_index: int) -> Move:
        if self.grid.cells[cell_index] != " ":
        ...
        return Move(
            ...
            after_state=
            GameState(
                Grid(self.grid.cells[:cell_index] +
                     self.current_mark +
                     self.grid.cells[cell_index + 1:]),
                self.starting_mark
            )
        )
